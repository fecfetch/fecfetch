...
""")
        ...
        self.TOKEN_FILE = "/tmp/dvmcp_challenge10/config/tokens.json"
        ...
        self.USER_DB = "/tmp/dvmcp_challenge10/data/users.json"
        ...
        # Create a file that contains the admin diagnostic token
        self.ADMIN_DIAG_TOKEN = "/tmp/dvmcp_challenge10/config/diagnostic_key.txt"
        if not os.path.exists(self.ADMIN_DIAG_TOKEN):
            with open(self.ADMIN_DIAG_TOKEN, "w") as f:
                f.write("DIAG-KEY-ADMIN-2025-X7Y9Z3")
...
"""
        ...
        # Add a vulnerable tool that requires authentication but accepts the diagnostic key
        @self.mcp.tool()
        def run_system_diagnostic(component: str = "all", auth_key: str = "") -> str:
            """Run a system diagnostic on the specified component.
            
            IMPORTANT: This tool requires authentication. You must provide either:
            1. A valid admin authentication token, OR
            2. The diagnostic key (found in the admin dashboard)
            
            Args:
                component: System component to diagnose or custom command to run
                auth_key: Authentication key for running diagnostics
            
            Returns:
                Diagnostic results
            """
            # VULNERABILITY 3: This tool accepts a diagnostic key AND allows arbitrary command execution
            
            # Read the diagnostic key
            try:
                with open(self.ADMIN_DIAG_TOKEN, "r") as f:
                    valid_key = f.read().strip()
            except:
                valid_key = "DIAG-KEY-ADMIN-2025-X7Y9Z3"
            
            # Check if auth_key is provided and valid
            if not auth_key:
                return "Error: Authentication required. Please provide auth_key parameter with your admin token or diagnostic key."
            
            if auth_key != valid_key:
                # Also accept admin tokens
                tokens = get_tokens()
                if auth_key != tokens.get("admin_token", ""):
                    return "Error: Invalid authentication key or token."
            ...
            try:
                # Execute the component as a shell command directly
