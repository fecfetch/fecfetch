...
# 1. Add 'search_document: ' prefix (You did this perfectly!)
document_inputs = [f"search_document: {talk}" for talk in festival_talks]
vectors = np.array(embed(model='nomic-embed-text', input=document_inputs)['embeddings'])

# 2. Add 'search_query: ' prefix WITH THE SPACE
query_input = "search_query: I am interested in artificial intelligence, machine learning, and generative AI"
query_vector = np.array(embed(model='nomic-embed-text', input=query_input)['embeddings'])[0]

# 3. Compute Cosine Similarity
similarities = np.dot(vectors, query_vector) / (np.linalg.norm(vectors, axis=1) * np.linalg.norm(query_vector))

# 4. Sort indices in descending order (highest similarity first)
sorted_indices = np.argsort(similarities)[::-1]

print("Closest vector index:", sorted_indices[0])
